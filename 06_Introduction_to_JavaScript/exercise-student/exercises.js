...
function frontAgain(char) {
	...
	if(char.charAt(0,2) == char.charAt(char.length -2, char.length)) {
		return true;
	} else {
		return false;
	} 
...
function filterMultiplesOfX(values, x) {
	...
	let newValues = []; //create an empty array that you're going to put the items in
	for(let i =0; i <values.length; i++) {
		//check if values[i] is evenly divisible by the value of x
		if (values[i] % x == 0) {
