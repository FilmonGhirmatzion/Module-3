...
export default {
  ...
  methods: {
    ...
    saveNewBoard() {
      ...
      // .then() is called on the Promise to set an anonymous function for it call when the promise fullfilled (successful)
      .then( response => {
      ...
      // .catch() is called on the Promise to set an anonymous function for it to call if the promise is rejected (fails)
      .catch( error => {
        if (error.response) {
          this.errorMsg = "Error Status " + error.response.statusText;
        } else if (error.request) {
          this.errorMsg = "Server did not respond";
        } else {
          this.errorMsg = "Something went very wrong!";
        }
        this.isLoading = false;
      });
    },
